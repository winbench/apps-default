...
if (Test-Path "$mariadbPath\mariadb.pdb") {
    del "$mariadbPath\mariadb.pdb"
}
...
if (!(Test-Path $dataDir -PathType Container)) {
    ...
    & "$mariadbPath\mariadb-install-db.exe" "--datadir=$dataDir"

    Write-Host ""
    Write-Host "Running SQL script for initialization..."
    $initFile = App-SetupResource "Bench.MariaDB" "init.sql"
    $serverProc = Start-Process "$mariadbPath\mariadbd.exe" -PassThru -NoNewWindow -ArgumentList @(
    ...
    do {
        & "$mariadbPath\mariadb-admin.exe" --silent ping | Out-Null
        $mariadbAdminExitCode = $LastExitCode
        $testCnt += 1
        [Threading.Thread]::Sleep(1)
    } while ($mariadbAdminExitCode -ne 0 -and $testCnt -le $testLimit)
    if ($testCnt -eq $testLimit) {
        Write-Host "TIMEOUT"
        Write-Warning "MariaDB server not responding. Database initialization may have failed."
    } else {
        Write-Host "OK"
        & "$mariadbPath\mariadb-admin.exe" --user=root --password=bench shutdown
